...
import { APP, EDITEUR } from "../data/editeur";
...
const LIENS_CONTACT: ILienContact[] = [
	{ icon: Mail, label: EDITEUR.email, href: `mailto:${EDITEUR.email}` },
	...
	return (
		<PageDocument titre="Contact">
			<p className="text-base leading-relaxed text-texte">
				Une question, une erreur repérée dans une fiche, une molécule qui vous
				manque ?
			</p>
			<p>
				Écrivez directement à Bolus, un réponse vous sera apportée sous 5 jours
				ouvrés.
			</p>
			...
			<nav className="overflow-hidden rounded-2xl divide-y divide-texte/10 bg-surface">
				{LIENS_CONTACT.map(({ icon: Icon, label, href, externe }) => (
					...
					>
						<Icon
							className="h-5 w-5 shrink-0 text-texte/70"
							aria-hidden="true"
						/>
						<span className="flex-1 text-base font-medium">{label}</span>
						<ChevronRight
							className="h-4 w-4 shrink-0 text-texte/40"
							aria-hidden="true"
						/>
					</a>
				))}
			...
			<BlocAvertissement icone={AlertTriangle} couleur="var(--alerte)">
				<div className="flex flex-col gap-2">
					<p className="text-sm leading-relaxed text-texte">
						...
						<br />
						Toute erreur signalée est vérifiée contre le RCP source avant
						correction.
					</p>
					{hrefErreur && (
						<a
