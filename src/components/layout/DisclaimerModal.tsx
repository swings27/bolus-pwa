...
export default function DisclaimerModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-encre/60 p-4">
      <div className="w-full max-w-md rounded-2xl bg-surface p-6 shadow-xl">
        <h2 className="text-lg font-semibold text-encre">Avertissement</h2>
        ...
        <label className="mt-4 flex cursor-pointer items-start gap-2 text-sm text-encre">
          <input
            type="checkbox"
            checked={caseCochee}
            onChange={(e) => setCaseCochee(e.target.checked)}
            className="mt-0.5 h-4 w-4 accent-terracotta-profond"
          />
        ...
        <button
          ...
          disabled={!caseCochee}
          onClick={accepter}
          className="mt-5 w-full rounded-lg bg-terracotta-profond py-3 text-sm font-medium text-surface disabled:cursor-not-allowed disabled:opacity-40"
        >
