...
export default function TableReferenceIMC() {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <span className="text-[0.625rem] font-semibold uppercase tracking-widest text-texte-doux">
            ...
            <tr>
              {REPERES.map(({ categorie }, index) => (
                <th
                  ...
                  className={`border-b border-texte/10 px-0.5 pb-1.5 pt-2 align-bottom text-center text-[0.59375rem] font-medium leading-tight text-texte ${index > 0 ? 'border-l border-texte/10' : ''}`}
                >
                  {categorie}
                </th>
              ))}
          ...
          <tbody>
            <tr>
              {REPERES.map(({ categorie, plage }, index) => (
                ...
                <td
                  key={categorie}
                  className={`px-0.5 pb-2 pt-1.5 text-center text-[0.6875rem] font-semibold tabular-nums leading-tight text-texte ${index > 0 ? 'border-l border-texte/10' : ''}`}
                >
                  {plage}
                </td>
              ))}
      ...
      <p className="text-[0.625rem] italic leading-relaxed" style={{ color: 'var(--texte-doux)' }}>
