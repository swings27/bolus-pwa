...
function Badge({ tonalite, children }: { tonalite: Tonalite; children: ReactNode }) {
  ...
  return (
    <span className="box-border w-[92px] shrink-0 rounded-full px-1.5 py-1 text-center text-xs font-semibold" style={style}>
      {children}
    </span>
  )
}
  ...
  return (
    <div className="flex flex-col">
      {donnees.recommandation_sonde && (
        ...
        >
          <div className="mb-1.5 text-etiquette font-semibold uppercase tracking-wide" style={{ color: 'var(--validation)' }}>
            Par sonde — forme à privilégier
          </div>
          <div className="text-sm font-semibold leading-snug text-texte">{donnees.recommandation_sonde.forme_preferee}</div>
          {donnees.recommandation_sonde.alternative && (
            <div className="mt-1.5 text-xs text-texte-doux/80">Alternative : {donnees.recommandation_sonde.alternative}</div>
          )}
        </div>
      )}
        ...
        {donnees.formes.map((forme, index) => (
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="text-xs font-semibold leading-snug text-texte">{forme.type}</div>
              {forme.dosage && <div className="text-xs text-texte-doux">{forme.dosage}</div>}
            </div>
