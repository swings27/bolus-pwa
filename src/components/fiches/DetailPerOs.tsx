...
function Badge({ tonalite, children }: { tonalite: Tonalite; children: ReactNode }) {
  ...
  return (
    <span className="box-border w-[92px] shrink-0 rounded-full px-1.5 py-1 text-center text-[0.65625rem] font-semibold" style={style}>
      {children}
    </span>
  )
}
  ...
  return (
    <div className="flex flex-col">
      {donnees.recommandation_sonde && (
        ...
        >
          <div className="mb-1.5 text-[0.59375rem] font-semibold uppercase tracking-wide" style={{ color: 'var(--validation)' }}>
            Par sonde — forme à privilégier
          </div>
          <div className="text-[0.8125rem] font-semibold leading-snug text-texte">{donnees.recommandation_sonde.forme_preferee}</div>
          {donnees.recommandation_sonde.alternative && (
            <div className="mt-1.5 text-[0.71875rem] text-texte-doux/80">Alternative : {donnees.recommandation_sonde.alternative}</div>
          )}
        </div>
      )}
        ...
        {donnees.formes.map((forme, index) => (
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="text-[0.71875rem] font-semibold leading-snug text-texte">{forme.type}</div>
              {forme.dosage && <div className="text-[0.65625rem] text-texte-doux">{forme.dosage}</div>}
            </div>
