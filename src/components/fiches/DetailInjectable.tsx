      ...
      {donnees.reconstitution && (
        <div>
          ...
          <div className="rounded-xl p-3.5" style={CARTE_STYLE}>
            ...
            {volumes.length > 0 && (
              <ul className="mt-1.5 flex flex-col gap-0.5 text-[0.71875rem] text-texte-doux">
            ...
            {donnees.reconstitution.stabilite_avant_dilution && (
              <p className="mt-2.5 border-t border-texte/10 pt-2 text-[0.6875rem] leading-relaxed text-texte-doux">
            ...
            {preparationsAdultes.map((prep, index) => (
              <div key={index} className="rounded-xl p-3.5" style={CARTE_STYLE}>
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  <span
                    className="rounded-md px-2 py-1 text-[0.625rem] font-semibold"
                    style={{ backgroundColor: 'var(--texte)', color: 'var(--fond)' }}
                  >
                    {LABELS_VOIE[prep.voie] ?? prep.voie}
                  </span>
                  {prep.duree && <span className="text-[0.71875rem] font-semibold text-texte">{prep.duree}</span>}
                  {prep.contexte && <span className="text-[0.65625rem] italic text-texte-doux">{prep.contexte}</span>}
                </div>
        ...
        <div className="mt-5">
          <BlocAvertissement couleur="var(--alerte)" titre="Incompatible en Y">
            {/* Liste repliée à 3 lignes comme les indications et les
                ...
                hors de l'écran. */}
            <TexteRepliable couleur="var(--alerte)" className="text-[0.78125rem] leading-relaxed text-texte">
